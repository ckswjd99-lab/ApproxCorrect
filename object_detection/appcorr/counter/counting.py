...
from .base import ExtendedModule, numeric_tuple
...
class MlpWithCountedLinear(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None,
        ...
        self.act = act_layer()  # GELU, ReLU, etc.
        self.fc2 = CountedLinear(hidden_features, out_features)

    def forward(self, x):
        x = self.fc1(x)
        x = self.act(x)         # <- CountedLinear doesn't include activation
        x = self.fc2(x)
        return x
...
class AddDecomposedRelPos(ExtendedModule):
    def __init__(self):
        super().__init__()
